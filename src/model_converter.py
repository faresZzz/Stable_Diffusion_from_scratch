...
def load_from_standard_weights(input_file: str, device: str) -> dict[str, torch.Tensor]:
    """
    Loads a standard Stable Diffusion model checkpoint and converts it to a custom format.
    This function takes a standard Stable Diffusion model checkpoint file and converts
    its weights and biases into a custom format with separate dictionaries for the diffusion
    model, encoder, decoder, and clip components. Each parameter from the original model is
    mapped to its corresponding parameter in the new structure.
    Args:
        input_file (str): Path to the standard Stable Diffusion model checkpoint file.
        device (str): The device to load the model onto ('cpu', 'cuda', etc).
    Returns:
        dict[str, torch.Tensor]: A dictionary containing the converted model parameters
        organized into 'diffusion', 'encoder', 'decoder', and 'clip' sub-dictionaries.
    Note:
        The conversion mapping is based on the structure described in:
    ...
    # Initialize the converted model dictionary
    converted = {}
    converted['diffusion'] = {}
    converted['encoder'] = {}
    converted['decoder'] = {}
    ...
    # Map the original model parameters to the new structure
